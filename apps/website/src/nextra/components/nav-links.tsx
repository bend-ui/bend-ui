...
const classes = {
  ...
  icon: cx('nx-inline nx-h-5 nx-shrink-0'),
};
...
export const NavLinks = ({
  ...
  return (
    ...
    >
      {prev && (
        <Anchor
          className={cx(classes.link, 'ltr:nx-pr-4 rtl:nx-pl-4')}
          href={prev.route}
          title={prev.title}
        >
          <ArrowRightIcon className={cx(classes.icon, 'ltr:nx-rotate-180')} />
          {prev.title}
        </Anchor>
      )}
      {next && (
        ...
        >
          {next.title}
          <ArrowRightIcon className={cx(classes.icon, 'rtl:nx-rotate-180')} />
        </Anchor>
      )}
    </div>
  );
