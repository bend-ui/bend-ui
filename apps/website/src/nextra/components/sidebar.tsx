...
const classes = {
  ...
  list: cx('nx-flex nx-flex-col nx-gap-1'),
  border: cx(
      ...
      <Collapse className="ltr:nx-pr-0 rtl:nx-pl-0 nx-pt-1" isOpen={open}>
        {Array.isArray(item.children) ? (
          <Menu
            anchors={anchors}
            base={item.route}
            className={cx(classes.border, 'ltr:nx-ml-3 rtl:nx-mr-3')}
        ...
        {hasMenu && (
          <div
            className={cx(
              ...
              showSidebar
                ? cx(hasI18n && 'nx-justify-end', 'nx-border-t')
          ...
          >
            <LocaleSwitch
              className={cx(showSidebar ? 'nx-grow' : 'max-md:nx-grow')}
              lite={!showSidebar}
            />
            {config.darkMode && (
              <div
