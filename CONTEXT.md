...
- **Distributed skill**: a portable agent skill whose canonical source lives under the repository's root `skills/<name>` publishing directory. Bend UI exposes its own Distributed skills to supported agents through symlinks such as `.agents/skills/<name>` and `.claude/skills/<name>` rather than maintaining copies.
